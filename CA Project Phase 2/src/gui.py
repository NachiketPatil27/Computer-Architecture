...
class Ui_MainWindow(object):
    ...
    def guihelp(self):
        webbrowser.open('https://github.com/AtulKharat256/Computer-Architecture')

    def run(self):
            data=open('output_log.mc','r')
            for i in data:
                self.textBrowser.append(i)

            data2 = open('register.mc','r')
            i = 0
            for item in data2:
                temp2=str(item)
    ...
    def opendata(self):
        try:
            ...
            subprocess.call([opener, "test_case.mc"])
    def outputlog(self):
        try:
            string=os.path.abspath(os.getcwd())
            string+="\\output_log.mc"
            os.startfile(string)
        except:
            opener = "open" if sys.platform == "darwin" else "xdg-open"
            subprocess.call([opener, "output_log.mc"])
